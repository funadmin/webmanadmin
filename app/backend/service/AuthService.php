...
//        $list[]['pid'] = $v['pid'];
    ...
    public function checkNode()
    {
        ...
        if (!in_array($controller, $cfg['noLoginController']) && !in_array($url, $cfg['noLoginNode'])) {
            if (!$this->isLogin()) {
                return $this->error(lang('Please Login First'), __u('login/index'));
            }
            if ($adminId && $adminId != $cfg['superAdminId']) {
                if (!in_array($controller, $cfg['noRightController']) &&  !in_array($url, $cfg['noRightNode'])) {
                    if (request()->isAjax() && $cfg['isDemo'] == 1) {
                        return $this->error(lang('Demo is not allow to change data'));
                    }
                    $this->hrefId = AuthRule::where('href', $url)
                        ->where('status', 1)
                        ->value('id');
                    $menuid = 0;
                    if(Str::endsWith($url,'/index')){
                        $menuid =  AuthRule::where('href', substr($url,0,strlen($url)-6))
                            ->where('status', 1)
                            ->value('id');
                    }
                    ...
                    if ($this->hrefId) {
                        ...
                        if (!in_array($url, $cfg['noRightNode'])) {
                            return $this->error(lang('Permission Denied'));
                        }
                    }
                }
                ...
                if (!in_array($controller, $cfg['noRightController']) && !in_array($url, $cfg['noRightNode'])) {
                    if (request()->isPost() && $cfg['isDemo'] == 1) {
                        return $this->error(lang('Demo is not allow to change data'));
                    }
                }
            }
            ...
            //不需要鉴权
            in_array($controller, $cfg['noLoginController'])
            //不需要登录
            && in_array($url, $cfg['noLoginNode'])
        ) {
            if ($this->isLogin()) {
                return true;
            }
        ...
        if (!in_array($controller, $cfg['noLoginController']) && !in_array($url, $cfg['noLoginNode'])) {
            ...
            if ($adminId && $adminId != $cfg['superAdminId']) {
                if (!in_array($controller, $cfg['noRightController']) && !in_array($url,$cfg['noRightNode'])) {
                    ...
                    if (in_array($url, $cfg['noRightNode'])) return true;
                }elseif(in_array($controller, $cfg['noRightController']) && in_array($url, $cfg['noRightNode'])){
                    return true;
                }
            } else {//超管
                return true;
            }
        } elseif (in_array($controller, $cfg['noLoginController'])
        && in_array($url, $cfg['noLoginNode'])){//不需要登录也就不需要鉴权了权限最大化
            return true;
        } elseif (in_array($controller, $cfg['noRightController']) && in_array($url, $cfg['noRightNode'])) {
            return true;
        }
